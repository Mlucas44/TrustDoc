/**
 * Analysis Restore API Route
 *
 * POST /api/analysis/[id]/restore - Restore a soft-deleted analysis (owner only)
 */
 ...
 *
 * Restore a soft-deleted analysis (idempotent)
 *
 * Security:
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    // 3. Restore (idempotent)
    try {
      await AnalysisRepo.restore(id, user.id);

      // Success: 204 No Content (idempotent)
    ...
    console.error("[POST /api/analysis/[id]/restore]", error);
