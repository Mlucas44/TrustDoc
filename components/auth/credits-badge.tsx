...
interface CreditsBadgeProps {
  /**
   * Optional credits value to display (overrides session credits)
   */
  credits?: number;
  /**
...
/**
 * Badge component displaying user credits
 *
 * @example
 * ```tsx
 * <CreditsBadge />
...
export function CreditsBadge({
  ...
  // Compute credits from props or session
  const creditsValue = propCredits ?? session?.user?.credits ?? null;
  const [apiCredits, setApiCredits] = useState<number | null>(null);

  useEffect(() => {
    // Only fetch from API if we have no credits and user is authenticated
    if (status === "authenticated" && creditsValue === null && apiCredits === null) {
      fetch("/api/me")
        .then((res) => res.json())
        .then((data) => {
        ...
        .catch((err) => {
          console.error("[CreditsBadge] Failed to fetch credits:", err);
        });
    }
  }, [status, creditsValue, apiCredits]);

  const credits = creditsValue ?? apiCredits;
  ...
  if (status === "unauthenticated" || credits === null) {
    return null;
  }
